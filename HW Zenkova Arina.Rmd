...
#dplyr::distinct()  #функция берёт в каждой группе (в данном случае по переменной "Группа") только первую строку
```{r}
data_frame %>%
...
##Испавить код (функция не  выполняется)
```{r} 
statistics <- list(
      `Количество субъектов` = ~length(.x),
      `Количество (есть данные)` = ~sum(!is.na(.x)),
...
tibble(`Adverse events` = c("soc желудочно-кишечные нарушения 10017947", "рт тошнота 10028813", "soc нарушения со стороны нервной системы 10029205", "рт головная боль 10019211")) %>%
  flextable() %>%
  theme_box() %>%
